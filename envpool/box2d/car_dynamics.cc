...
void Car::step(float dt) {
  for (auto w: wheels) {
    ...
    for (auto t: w->tiles) {
      friction_limit = max(
          friction_limit, kFrictionLimit * t->roadFriction
      );
      grass = false;
    }
    // Force
    ...
    w->omega += (
        dt * kEnginePower * w->gas / kWheelMomentOfInertia/ (abs(w->omega) + 5.0)
    );
    ...
    if (abs(force) > 2.0 * friction_limit) {
        if (w->skid_particle != NULL && w->skid_particle->isGrass == grass && w->skid_particle->poly.size() < 30) {
          w->skid_particle->poly.push_back({w->wheel->GetPosition().x, w->wheel->GetPosition().y});
        } else if (w->skid_start == NULL) {
          w->skid_start = new b2Vec2(w->wheel->GetPosition());
        } else {
          w->skid_particle = createParticle(*(w->skid_start), w->wheel->GetPosition(), grass);
          w->skid_start = NULL;
        }
    } else {
      w->skid_start = NULL;
