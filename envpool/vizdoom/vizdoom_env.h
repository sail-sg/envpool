...
class VizdoomEnv : public Env<VizdoomEnvSpec>, public RenderableEnv {
 ...
 public:
  VizdoomEnv(const Spec& spec, int env_id)
    ...
    // Preserve higher scenario-configured resolutions, but clamp older
    // low-res scenarios up to the documented default when needed.
    if (dg_->getScreenWidth() < 320) {
    ...
    raw_buf_ = Array(FrameSpec({dg_->getScreenHeight(), dg_->getScreenWidth(), 1}));
    render_img_ =
