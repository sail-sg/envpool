...
"""Unit tests for Mujoco gym v3 environments."""
...
ld_path = ":" + os.path.join(
  "/home", os.environ["USER"], ".mujoco", "mujoco210", "bin"
)
ld_path += ":/usr/lib/nvidia"  # if the runner has Nvidia GPU
os.environ["LD_LIBRARY_PATH"] = os.environ.get("LD_LIBRARY_PATH", "") + ld_path
...
class _MujocoEnvPoolTest(absltest.TestCase):
  ...
  def run_align_check(
    ...
    for i in range(5):
      ...
      self.reset_state(env0.unwrapped, info["qpos0"][0], info["qvel0"][0])
  ...
  def test_humanoid_standup(self) -> None:
    env0 = gym.make("HumanoidStandup-v2")
    env1 = HumanoidStandupGymEnvPool(
      HumanoidStandupEnvSpec(
        HumanoidStandupEnvSpec.gen_config(
          gym_reset_return_info=True,
          post_constraint=False,
        )
      )
  ...
  def test_inverted_double_pendulum(self) -> None:
    env0 = gym.make("InvertedDoublePendulum-v2")
  ...
  def test_inverted_pendulum(self) -> None:
    ...
    env1 = InvertedPendulumGymEnvPool(
      InvertedPendulumEnvSpec(
        InvertedPendulumEnvSpec.gen_config(
          gym_reset_return_info=True,
          post_constraint=False,
        )
      )
  ...
  def test_pusher(self) -> None:
    env0 = gym.make("Pusher-v2")
    env1 = PusherGymEnvPool(
      PusherEnvSpec(
        PusherEnvSpec.gen_config(
          gym_reset_return_info=True,
          post_constraint=False,
        )
      )
  ...
  def test_reacher(self) -> None:
    env0 = gym.make("Reacher-v2")
    env1 = ReacherGymEnvPool(
      ReacherEnvSpec(
        ReacherEnvSpec.gen_config(
          gym_reset_return_info=True,
          post_constraint=False,
        )
      )
