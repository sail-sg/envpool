...
reference is the upstream Python renderer. For Google Research Football, the
reference is an independent oracle built on the official engine and upstream
scenario semantics. For Atari, Procgen, and VizDoom, the reference is the
exact in-tree render oracle used by the test suite.
