...
oracle and the same action sequence used by the render test. If EnvPool's
public API auto-resets a task within those three calls, the official oracle is
reset at that same reset boundary and synchronized only to the corresponding
reset state. The nine upstream Bimanual/Soccer IDs listed above remain
registered but are omitted from these official render sheets for the same
oracle instantiation failure.
